/* Pergutas:
Se fosse paas teria como editar endereços de rede, firewall e etc?
A edição pode ser feita via terraform ou painel de azure?

IP publico criado é aleatorio? tem como editar ou usar uma mascara?
 Terraform tem gera erro por identação?
...
terraform {
  ...
  //local que será usado https://registry.terraform.io/browse/providers
  //neste caso será usado o azure e o id dele é azurerm
  required_providers {
    azurerm = {
...
provider "azurerm" {
  //caso vc ja tenha feito login coloca pra ele pular o login da forma que esta aqui embaixo
...
//primeira criação de um recurso (resource), criando um resource group que é como se fosse uma pasta dentro do azure
...
resource "azurerm_virtual_network" "vnet-aulainfra" {
  name                = "vnet-aula"
  //criada dentro com referencia ao resource de cima que seria a 'pasta' onde ele esta
...
//criando uma sub rede
 resource "azurerm_subnet" "sub-aulainfra" {
  name                 = "sub-aula"
  //resource group da subrede
  resource_group_name  = azurerm_resource_group.rg-aulainfracloud.name
  //maquina virtual ou rede da subrede
